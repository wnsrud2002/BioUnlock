...
public enum PalmConfig {
    ...
    /// 랜드마크 기반 경로에서만 쓴다(현재 손금 인증은 초근접 경로를 쓴다).
    public static var minSourcePixels: CGFloat = 450

    // MARK: - 초근접 손금 경로

    /// 살색 픽셀 비율 하한. 손이 아니라 벽·책상을 비추는 걸 걸러낸다.
    /// 느슨하게 잡는다 — 피부색 폭이 넓어 빡빡하면 정당한 사용자를 막는다.
    /// 진짜 품질 게이트는 아래 텍스처 쪽이다.
    public static var minSkinFraction: Float = 0.55

    /// ROI 유효 픽셀 비율 하한. 주름 텍스처가 실제로 잡혔는지를 본다.
